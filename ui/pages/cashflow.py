...
def render(result: ModelResult, assumptions: Assumptions) -> None:
    ...
    st.table(
        [
            ...
            {"Parameter": "Opening Cash Balance", "Value": _format_amount(assumptions.cashflow.opening_cash_balance_eur), "Unit": "EUR", "Notes": "Opening cash balance."},
        ]
    )
    st.markdown("Consolidated cashflow statement (5-year plan)")
