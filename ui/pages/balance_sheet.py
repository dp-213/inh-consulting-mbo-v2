...
def render(result: ModelResult, assumptions: Assumptions) -> Assumptions:
    ...
    with st.expander("Explain business & calculation logic", expanded=False):
        st.markdown(
            "**1) Business Question**\n"
            "Does the balance sheet remain internally consistent and solvent throughout the plan?\n\n"
            "**2) What This Shows / What This Does NOT Show**\n"
            "Shows the evolution of cash, fixed assets, debt, pensions, and equity, plus a structural consistency check.\n"
            "Does not show detailed working capital accounts or valuation; it is a solvency and integrity check, not a reporting balance sheet.\n\n"
            "**3) Calculation Logic (Transparent, Step-by-Step)**\n"
            "Cash follows the closing cash position from the cashflow statement.\n"
            "Fixed assets grow by capex and shrink by depreciation over time.\n"
            "Total assets combine cash and fixed assets; total liabilities combine financial debt and pension obligations.\n"
            "Equity rolls forward from the prior year, adding net income and equity injections, and subtracting dividends and buybacks.\n"
            "The consistency check requires total assets to match total liabilities plus equity each year.\n\n"
            "**4) Interpretation for the Decision**\n"
            "Negative equity or rising leverage without earnings improvement signals structural fragility.\n"
            "Persistent negative cash indicates a funding gap, not a timing issue.\n\n"
            "**5) Insights & Red Flags**\n"
            "Equity erosion combined with high pension liabilities materially reduces solvency.\n"
            "Debt reduction without cash improvement often masks operating weakness.\n"
            "If assets and liabilities do not reconcile, the model is not reliable for decision-making.\n\n"
            "**6) Key Dependencies**\n"
            "Net income from the P&L, opening equity, debt schedule balances, cashflow closing cash, depreciation rate, pension obligations."
        )
