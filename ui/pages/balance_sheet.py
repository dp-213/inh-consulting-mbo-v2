...
def render(result: ModelResult, assumptions: Assumptions) -> Assumptions:
    ...
    with st.expander("Explain business & calculation logic", expanded=False):
        st.markdown(
            "**1) Business Question**\n"
            "- Is the balance sheet internally consistent and solvent across the plan?\n"
            "**2) What This Shows / What This Does NOT Show**\n"
            "- Shows cash, fixed assets, debt, pensions, and equity evolution with a consistency check.\n"
            "- Does not show detailed working capital accounts or valuation.\n"
            ...
            "- Equity End = Equity Start + Net Income + Equity Injections − Dividends − Buybacks.\n"
            "- Consistency requires Assets = Liabilities + Equity each year.\n"
            "**4) Interpretation for the Decision**\n"
            "- Negative equity or rising leverage indicates structural fragility.\n"
            "- Persistent negative cash is a funding gap, not timing noise.\n"
            "**5) Insights & Red Flags**\n"
            "- Equity erosion plus high pensions materially reduces solvency.\n"
            "- Debt reduction without cash improvement masks operating weakness.\n"
            "- Any reconciliation gap invalidates the model for decisions.\n"
            "**6) Key Dependencies**\n"
            "- Net income, opening equity, debt balances, closing cash, depreciation rate, pension obligations."
        )
